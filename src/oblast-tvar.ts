/**
 * Tvar oblasti a test „leží bod uvnitř?" — čistý počet bez databáze.
 *
 * Proč zvlášť od `oblast.ts`: tenhle výpočet potřebuje i webová aplikace,
 * aby při kreslení rovnou ukazovala, kolik firem do tvaru padne. Kdyby si
 * ho opsala, dvě kopie by se dřív nebo později rozešly a mapa by ukazovala
 * něco jiného, než co se pak uloží. Proto tu nesmí přibýt nic, co běží
 * jen v Node — soubor musí zůstat spustitelný i v prohlížeči.
 *
 * Proč se to nepočítá v databázi: sdílený Postgres by uměl PostGIS, ale
 * testy běží nad PGlite, která ho nemá — dotaz opřený o PostGIS by rozbil
 * `npm test` bez sítě. Tvar se proto ukládá jako obyčejná data a bod se
 * testuje v kódu. Při našich objemech (celá ČR = 35 tisíc firem nad
 * 25 zaměstnanců) je to otázka milisekund a chová se to všude stejně.
 */
